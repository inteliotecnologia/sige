...
function pega_prioridade_rm($dias) {
	...
	switch ($dias) {
		case 0: return("Baixa"); break;
		case 1: return("Média"); break;
		case 2: return("Alta"); break;
		case 5: return("Urgente"); break;
	}
}
...
function pega_nota_situacao_atual_os($id_os) {
	$result= mysql_query("select nota from man_oss_andamento
							where id_os= '$id_os'
							and   id_situacao = '5'
							order by id_os_andamento desc limit 1
							") or die(mysql_error());
	$rs= mysql_fetch_object($result);
...
function pega_descricao_contento($nota) {
	switch($nota) {
		case '0':
			$bg= "bg_vermelho";
			$txt= "Não contento";
...
function pega_situacao_os($tipo) {
	$vetor= array();
...
function pega_tipo_servico($id_servico_tipo) {
	$rs= mysql_fetch_object(mysql_query("select servico_tipo from man_servicos_tipos
											where id_servico_tipo = '$id_servico_tipo'
											"));
	return($rs->servico_tipo);
}
...
function excluido_ou_nao($var) {
	...
	else $retorno_msg= "Não foi possível excluir! Item sendo usado.";
...
function converte_data_completa_utc($data_local) {
	...
	if (!strpos($data, "-")) {
		$data_local= formata_data($data_local);
	}
	else {
	
	}
	
	$data_utc = new DateTime($data_local, new DateTimeZone('America/Sao_Paulo'));
