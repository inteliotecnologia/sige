...
if (pode_algum("ps", $_SESSION["permissao"])) {
	...
	if ($goma!="") $str .= " and   goma= '". $goma ."' ";
	
	if ($_POST["id_turno"]!="") $id_turno= $_POST["id_turno"];
	if ($_GET["id_turno"]!="") $id_turno= $_GET["id_turno"];
	if ($id_turno!="") $str .= " and   id_turno= '". $id_turno ."' ";
	
	$result= mysql_query("select * from op_suja_pesagem, op_suja_remessas
							where op_suja_pesagem.id_empresa = '". $_SESSION["id_empresa"] ."' 
							and   op_suja_pesagem.id_remessa = op_suja_remessas.id_remessa
							". $str ."
							order by op_suja_remessas.data_remessa desc, op_suja_pesagem.data_pesagem desc,
							op_suja_pesagem.hora_pesagem desc, op_suja_pesagem.id_pesagem desc
							");
...
<h2>Área suja - Pesagens</h2>

<ul class="recuo1">
	<li><a href="./?pagina=op/pesagem_suja&amp;acao=i">inserir</a></li>
</ul>
<br />
...
if ($num_paginas > 1) {
	...
	for ($i=0; $i<$num_paginas; $i++) {
		...
		if ($num_pagina==$i)
			...
			echo " <a href=\"./?pagina=op/pesagem_suja_listar&amp;data=". $data2 ."&amp;periodo=". $periodo ."&amp;goma=". $goma ."&amp;id_cliente=". $id_cliente ."&amp;id_turno=". $id_turno ."&amp;num_pagina=". $i. "\">". $link ."</a>";
	}
	*/
...
<fieldset class="tamanho200">
    ...
    <div class="flutuar_esquerda espaco_dir"><strong>Página:</strong></div> <input onkeyup="if (event.keyCode==13) pulaParaPagina('./?pagina=op/pesagem_suja_listar&amp;data=<?=$data2;?>&amp;periodo=<?=$periodo;?>&amp;goma=<?=$goma;?>&amp;id_cliente=<?=$id_cliente;?>&amp;id_turno=<?=$id_turno;?>&amp;num_pagina=', this);" class="tamanho40 flutuar_esquerda" name="num_pagina" id="num_pagina" value="<?= $num_pagina_real; ?>" /> de <b><?= fnumf($num_paginas); ?></b>.
</fieldset>
<? } ?>
...
<table cellspacing="0" width="100%">
	...
	<tr id="linha_<?=$i;?>" class="<?= $classe; ?> corzinha">
		<td align="center"><?= $rs->id_pesagem; ?></td>
		...
		<td align="center"><?= desformata_data($rs->data_pesagem) ." ". $rs->hora_pesagem; ?></td>
		<td align="center"><?= fnum($rs->peso); ?> kg</td>
		<td align="center"><?= $rs->hampers; ?></td>
		<td align="center">
			<? if ($_SESSION["id_departamento_sessao"]!="") { ?>
            <button class="tamanho50 espaco_dir" onclick="window.top.location.href='./?pagina=op/pesagem_suja&amp;acao=e&amp;id_pesagem=<?= $rs->id_pesagem; ?>';" id="link_edita<?=$i;?>">edita</button>
            <button class="tamanho50" onclick="var confirma= confirm('Tem certeza que deseja excluir?'); if (confirma) ajaxLink('conteudo', 'pesagemSujaExcluir&amp;id_pesagem=<?= $rs->id_pesagem; ?>');" id="link_exclui<?=$i;?>">exclui</button>
            <? } else { ?>
            <a href="./?pagina=op/pesagem_suja&amp;acao=e&amp;id_pesagem=<?= $rs->id_pesagem; ?>">
				<img border="0" src="images/ico_lapis.png" alt="Edita" /></a>
			|
			<a href="javascript:ajaxLink('linha_<?=$i;?>', 'pesagemSujaExcluir&amp;id_pesagem=<?= $rs->id_pesagem; ?>');" onclick="return confirm('Tem certeza que deseja excluir esta pesagem?');">
