...
if (pode("ps", $_SESSION["permissao"])) {
	...
	if ($_POST["tipo_relatorio"]=='d') {
		...
		if ($_POST["id_equipamento"]!="") $str_equi= "and   op_equipamentos.id_equipamento = '". $_POST["id_equipamento"] ."' ";
		...
		while ($rs_equi = mysql_fetch_object($result_equi)) {
			...
			$result_equi_teste= mysql_query("select op_suja_lavagem.id_lavagem from op_suja_lavagem, op_suja_lavagem_cestos, op_suja_remessas
	...
	//mensal
			...
			if (mysql_num_rows($result_equi_teste)>0) {
				...
				while ($rs_proc= mysql_fetch_object($result_proc)) {
					...
					if (mysql_num_rows($result_proc_teste)>0) {
						...
						$pdf->Cell(10, 0.5, "CLIENTE", 1, 0, "L", 1);
						$pdf->Cell(4, 0.5, "PESO", 1, 0, "C", 1);
						$pdf->Cell(0, 0.5, "MAQUINADAS", 1, 1, "C", 1);
						...
						while ($rs_cliente= mysql_fetch_object($result_cliente)) {
							...
							if (mysql_num_rows($result_cliente_teste)>0) {
								...
								$result_peso= mysql_query("select sum(op_suja_lavagem_cestos.peso) as peso_total, count(op_suja_lavagem.id_lavagem) as total_maquinadas
															from op_suja_lavagem, op_suja_lavagem_cestos, op_suja_remessas
															where op_suja_lavagem.id_empresa = '". $_SESSION["id_empresa"] ."'
															and   op_suja_lavagem.id_lavagem = op_suja_lavagem_cestos.id_lavagem
															and   op_suja_lavagem.id_equipamento = '". $rs_equi->id_equipamento ."'
								...
								$pdf->Cell(10, 0.5, " ". $rs_cliente->apelido_fantasia, 1, 0, "L", $fill);
								$pdf->Cell(4, 0.5, " ". fnum($rs_peso->peso_total) ." kg", 1, 0, "C", $fill);
								$pdf->Cell(0, 0.5, fnumf($rs_peso->total_maquinadas), 1, 1, "C", $fill);
								
								$peso_total_processo+= $rs_peso->peso_total;
								$total_maquinadas_processo+= $rs_peso->total_maquinadas;
								
								$peso_total_mes+= $rs_peso->peso_total;
								$total_maquinadas_mes+= $rs_peso->total_maquinadas;
						...
						if ($_POST["id_cliente"]=="") {
							...
							$result_peso_variadas= mysql_query("select sum(op_suja_lavagem_cestos.peso) as peso_total, count(op_suja_lavagem.id_lavagem) as total_maquinadas
														from op_suja_lavagem, op_suja_lavagem_cestos, op_suja_remessas
														where op_suja_lavagem.id_empresa = '". $_SESSION["id_empresa"] ."'
														and   op_suja_lavagem.id_lavagem = op_suja_lavagem_cestos.id_lavagem
														and   op_suja_lavagem.id_equipamento = '". $rs_equi->id_equipamento ."'
							...
							if ($rs_peso_variadas->peso_total>0) {
								...
								$pdf->Cell(10, 0.5, "  RELAVE", 1, 0, "L", $fill);
								$pdf->Cell(4, 0.5, fnum($rs_peso_variadas->peso_total) ." kg", 1, 0, "C", $fill);
								$pdf->Cell(0, 0.5, fnumf($rs_peso->total_maquinadas), 1, 1, "C", $fill);
						...
						$pdf->Cell(4, 0.5, fnum($peso_total_processo) ." kg", 1, 0, "C", 0);
						$pdf->Cell(0, 0.5, fnumf($total_maquinadas_processo), 1, 1, "C", 0);
		...
		$pdf->SetFont('ARIAL_N_NEGRITO', '', 10);
		$pdf->Cell(3.9, 0.6, "TOTAL DE MAQUINADAS:", 0, 0, 'L', 0);
		$pdf->SetFont('ARIALNARROW', '', 10);
		$pdf->Cell(3, 0.6, fnumf($total_maquinadas_mes), 0, 1, 'L', 0);
		
	}
	
	$pdf->AliasNbPages(); 
	$pdf->Output("lavagem_processo_relatorio_". date("d-m-Y_H:i:s") .".pdf", "I");
}
?>