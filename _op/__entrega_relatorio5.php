...
if (pode("pl12", $_SESSION["permissao"])) {
	...
	//está gerando a partir da lista de percursos
	if ($origem==2) {
	...
	*/
	...
	for ($i=0; $i<2; $i++) {
		...
		//pegar a última coleta efetuada dentro do prazo
		...
		if ($linhas_percurso>0) {
			...
			//pegar a entrega normal anterior a esta, para ter como parâmetro
			...
			if ($rs_contrato->basear_peso=="2") {										
				...
				if ( ($peso_coleta_temp==0) || ($peso_coleta_temp=="") || ($peso_coleta_temp=="0")  || ($peso_coleta_temp=="0,00") || ($peso_coleta_temp==NULL) )
					die("O peso da coleta ainda não foi lançado para este cliente/nota.<br><br><a target='_blank' href='./?pagina=transporte/percurso_dados&id_percurso=". $rs_coleta_ref->id_percurso ."'>Clique aqui</a> para cadastrar o peso.<br><br>Após cadastrar o peso, dê F5 nesta página para gerar o relatório.");
				
				//if ($_SESSION["id_usuario"]==13) { echo $rs_coleta_ref->id_percurso ." - ". $rs_coleta_ref->peso; die("<-- ");
			}
				
			//se encontrou uma última entrega normal para ter como referência…
			...
			//é a primeira entrega ever, o sistema vai fazer a conta de pesagens até um dia antes para ter um começo
			else {
		...
		//nao tem nenhuma entrega no dia em questão, dá um erro
		else {
		...
		//se o peso final for calculado pela suja empresa e não for suja cliente (coleta)
		...
		$pdf->Cell(8.5, 0.6, "DATA: ". $data_de_entrega ." | ENTREGA ". $tit_extra ." Nº ". $entrega, 1, 1, "L", 0);
