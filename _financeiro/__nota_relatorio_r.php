...
if (pode("i", $_SESSION["permissao"])) {
	...
	$pdf->SetAutoPageBreak(true, 2.35);
	...
	*/
	...
	if (($data1_pagamento!="") || ($data2_pagamento!="")) {
		$periodo_nota_pagamento= "RECEBIMENTO: ";
	...
	if ($_POST["id_cedente"]!="") {
		...
		$pdf->Cell(2.5, 0.6, $txt_cedente .":", 0, 0, 'L', 0);
		
		$pdf->SetFont('ARIALNARROW', '', 8);
		$pdf->Cell(0, 0.6, pega_pessoa($_POST["id_cedente"]), 0, 1, 'L', 0);
	...
	while ($rs= mysql_fetch_object($result)) {
		...
		if (strlen($pessoa_aqui)>44) $nome_pessoa_aqui= substr($pessoa_aqui, 0, 44) ."...";
		...
		if ($status_nota==1) {
			...
			if ($data1_pagamento!="") $str_pag = " and data_pagamento >= '$data1_pagamento'
													and   data_pagamento <= '$data2_pagamento'
													";
		...
		if ( ($linhas==$j) && ($_POST["agrupar"]!="") ) {
			...
			$pdf->Cell(2.5, 0.4, "". $tit3 .":", 0, 0, 'L', 0);
			$pdf->SetFont('ARIALNARROW', '', 7);
			$pdf->Cell(3, 0.4, "R$ ". fnum($valor_total_pago_cedente[$id_cedente[$i]]), 0, 1, 'L', 0);
	...
	$pdf->SetFont('ARIAL_N_NEGRITO', '', 8);
	$pdf->Cell(2.5, 0.5, "TOTAL ". $tit6 .":", 0, 0, 'L', 0);
	$pdf->SetFont('ARIALNARROW', '', 8);
