...
<h2>Busca de duplicata <?= $tit2; ?> (centro de custo por fornecedor)</h2>
...
<div id="conteudo_interno">
    <fieldset>
        ...
        <?
        if ($_GET["geral"]==1) {
        	if (($_GET["tipo_nota"]=="p") && ($_GET["status_nota"]=="1")) $aponta= "nota_relatorio_p_cc";
        	else $aponta= "nota_relatorio_r";
        ?>
        <form action="index2.php?pagina=financeiro/<?= $aponta; ?>" target="_blank" method="post" name="formNotaBuscar" id="formNotaBuscar">
        <? } else { ?>
        <form action="./?pagina=financeiro/nota_listar" method="post" name="formNotaBuscar" id="formNotaBuscar" onsubmit="return validaFormNormal();">
        <? } ?>

            <div class="parte50">
                
                <input class="escondido" type="hidden" id="validacoes" value="num_nota@vazio" />
                ...
                <input class="escondido" type="hidden" name="status_nota" value="<?= $status_nota; ?>" />
                
                <? if ($_GET["geral"]!=1) { ?>
                <label for="num_nota">Número da nota:</label>
                <input name="num_nota" id="num_nota" class="tamanho25p espaco_dir" value="" title="Número da nota" />
                <br /><br />
                
                <label for="valor_exato">Valor exato:</label>
                <input name="valor_exato" id="valor_exato" class="tamanho25p espaco_dir" value="" title="Valor exato" title="Valor da parcela"  onkeydown="formataValor(this,event);" />
                <br /><br />
                <? } ?>
                
                <? //if (($_GET["tipo_nota"]!="p") && ($_GET["status_nota"]!="1")) { ?>
                ...
                <select name="periodo" id="periodo" title="Período">	  		
                    ...
                    $result_per= mysql_query("select distinct(DATE_FORMAT(data_vencimento, '%m/%Y')) as data2
												from fi_notas_parcelas order by data_vencimento desc ");
                ...
                <? if (($tipo_nota=="r") && ($status_nota==1)) { ?>
                <label for="periodo_pagamento">Recebimento:</label>
                <select name="periodo_pagamento" id="periodo_pagamento" title="Período">	  		
					<option value="">--- TODOS ---</option>
					<?
					$i=0;
            ...
            <div class="parte50">
                ...
                <? if ($_GET["geral"]==1) { ?>
                <label>&nbsp;</label>
                <input type="checkbox" class="tamanho20" name="agrupar" id="agrupar" value="1" checked="checked" />
                <label for="agrupar" class="nao_negrito alinhar_esquerda tamanho300">Agrupar <?= $txt_cedente_plural; ?></label>
                <br />
                <? } ?>
                <br />
