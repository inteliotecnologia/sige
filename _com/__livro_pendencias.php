		...
		case 5:
			...
			if ($_GET["tipo_pendencia"]==1) {
				...
				if ($_GET["parte_pendencia"]==1) {
					$str_livro.= " and   ( com_livro_permissoes.id_departamento = '". $id_departamento_usuario2 ."' ";
					...
					while ($rs_permissao2= mysql_fetch_object($result_permissao2)) {
						$str_livro.= " or   com_livro_permissoes.id_departamento = '". $rs_permissao2->id_departamento ."' ";
					}
			...
			if ($_GET["id_departamento"]!="0") $str_livro_depto = " and   com_livro_permissoes.id_departamento = '". $_GET["id_departamento"] ."' ";
			
			$result_pendencias_livro= mysql_query("select distinct(com_livro.id_livro),
													com_livro.num_livro, com_livro.tipo_de, com_livro.de, com_livro.mensagem, com_livro.data_livro, com_livro.hora_livro,
													com_livro.tipo_resposta, com_livro.resposta, com_livro.resposta_id_livro, com_livro.id_motivo, com_livro.id_departamento_principal,
													com_livro.prioridade_dias, com_livro.reclamacao_original, com_livro.reclamacao_original_id_livro, com_livro.reclamacao_id_cliente,
													DATE_FORMAT(com_livro.data_livro, '%Y') as ano
													from com_livro, com_livro_permissoes
													where com_livro.id_empresa = '". $_SESSION["id_empresa"] ."'
													and   com_livro.id_livro = com_livro_permissoes.id_livro
													and   com_livro_permissoes.resposta_requerida_depto= '1'
													$str_livro
													$str_livro_depto
													and   com_livro.id_motivo <> '34'
													and   com_livro.id_motivo <> '37'
													and   com_livro.id_motivo <> '41'
													and   com_livro.id_motivo <> '42'
													order by  com_livro.id_livro desc
													") or die(mysql_error());
			
			$linhas_pendencias_livro= mysql_num_rows($result_pendencias_livro);
			
			//echo $linhas_pendencias_livro;
			
			if ($linhas_pendencias_livro>0) {
				
				//if ($_GET["parte_pendencia"]=="1") $campo_retornar= "com_livro.de";
				//else
				$campo_retornar= "com_livro_permissoes.id_departamento";
				
				$result_pendencias_deptos= mysql_query("select distinct(". $campo_retornar .")
														from com_livro, com_livro_permissoes
														where com_livro.id_empresa = '". $_SESSION["id_empresa"] ."'
														and   com_livro.id_livro = com_livro_permissoes.id_livro
														and   com_livro_permissoes.resposta_requerida_depto= '1'
				...
				/*if ($_GET["parte_pendencia"]=="1") {
					...
					while ($rs_pendencias_deptos= mysql_fetch_object($result_pendencias_deptos)) {
				...
				if ($_GET["parte_pendencia"]=="5") {
			?>
            
                <div class="div_abas screen" id="aba_pendencias_<?= $_GET["parte_pendencia"]; ?>">
                    <ul class="abas">
                        <li id="aba_deptos_todos" <? if ($_GET["id_departamento"]=="0") { ?>class="atual"<? } ?>><a href="javascript:void(0);" onclick="atribuiAbaAtual('aba_deptos_todos', 'aba_pendencias_<?= $_GET["parte_pendencia"]; ?>'); checaPendencias2('<?= $_GET["tipo_pendencia"]; ?>', '<?= $_GET["parte_pendencia"]; ?>', '0');">Todos</a></li>
                        
                        <?
                        $q=0;
                        while ($rs_pendencias_deptos= mysql_fetch_object($result_pendencias_deptos)) {
                        ?>
                        <li id="aba_minhas_<?=$q;?>" <? if ($_GET["id_departamento"]==$rs_pendencias_deptos->id_departamento) { ?>class="atual"<? } ?>><a href="javascript:void(0);" onclick="atribuiAbaAtual('aba_minhas_<?=$q;?>', 'aba_pendencias_<?= $_GET["parte_pendencia"]; ?>'); checaPendencias2('<?= $_GET["tipo_pendencia"]; ?>', '<?= $_GET["parte_pendencia"]; ?>', '<?= $rs_pendencias_deptos->id_departamento; ?>');"><?= pega_departamento($rs_pendencias_deptos->id_departamento); ?></a></li>
                        <? $q++; } ?>
                    </ul>
                </div>
                <br />
                ...
                <div id="conteudo_<?= $_GET["parte_pendencia"]; ?>">
                    <table cellspacing="0" width="100%">
                        <tr>
                          <th width="9%" align="left" valign="bottom">Data</th>
                          <th width="19%" align="left" valign="bottom">Identifica&ccedil;&atilde;o</th>
                          <th width="59%" align="left" valign="bottom">Mensagem</th>
                        ...
                        <tr class="<?= $classe; ?>">
                            <td valign="top">
                                <?= desformata_data($rs_pendencias_livro->data_livro) ."<br />". $rs_pendencias_livro->hora_livro . $novo; ?>
                            </td>
                            <td valign="top">
                            <strong>De:</strong>
                            <?
                                ...
                                <div id="livro_<?= $rs_pendencias_livro->id_livro; ?>">
                                    ...
                                    if (($rs_pendencias_livro->resposta!="0") && ($rs_pendencias_livro->resposta!="")) {
                                        if ($rs_pendencias_livro->tipo_resposta=="f") {
											...
											if ($rs_pendencias_livro->resposta=="0") {
                                    ...
                                    <? if ($_SESSION["tipo_usuario"]=="a") { ?>
                                    <a href="link.php?livroExcluir&amp;id_livro=<?= $rs_pendencias_livro->id_livro; ?>&amp;data=<?= desformata_data($rs_pendencias_livro->data_livro); ?>" onclick="return confirm('Tem certeza que deseja excluir?');">
                                    ...
                                    <div id="livro_resposta_<?=$rs_pendencias_livro->id_livro;?>">
                                        <a href="javascript:void(0);" onclick="respondeLivro('<?= $rs_pendencias_livro->id_livro; ?>', '<?= $id_departamento; ?>', '<?=$rs_pendencias_livro->tipo_de;?>', '<?=$rs_pendencias_livro->de;?>', '<?=$minhas;?>', '<?=$id_funcionario;?>', '<?=$parte;?>', '<?=$id_motivo;?>', '<?=$resposta_requerida;?>', '<?=desformata_data($data1);?>', '<?=desformata_data($data2);?>', '<?=$data;?>', '<?=$depto_para;?>', '<?=$id_departamento_principal;?>');">responder</a>
                                    </div>
                                </div>
                            </td>
